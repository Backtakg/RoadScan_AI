...
class HistoryStore:
    ...
    def _init(self) -> None:
        with self._connect() as conn:
            conn.execute("""CREATE TABLE IF NOT EXISTS inspections (
                ...
                route_distance_m REAL NOT NULL DEFAULT 0,
    ...
    def save(self, started_at: str, ended_at: str, frames: int, summary: dict[str, Any], analytics: dict[str, Any], events: list[dict[str, Any]], route: dict[str, Any]) -> str:
        ...
        with self._connect() as conn:
            conn.execute("INSERT INTO inspections VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?)", (
                inspection_id, started_at, ended_at, frames, summary.get("total", 0), summary.get("high", 0), summary.get("medium", 0), summary.get("low", 0), route.get("pointCount", 0), route.get("distanceMeters", 0), json.dumps(analytics), json.dumps(events), json.dumps(route),
            ))
            conn.commit()
    ...
    def list(self, limit: int = 50) -> list[dict[str, Any]]:
        ...
        for row in rows:
            item = dict(row)
            item["analytics"] = json.loads(item.pop("analytics_json"))
            output.append(item)
        return output
