...
app = FastAPI(title="RoadScan AI Vision API", version="0.6.0")
...
def health() -> dict[str, Any]:
    return {"status": "ok", "model": MODEL_PATH, "model_exists": os.path.exists(MODEL_PATH), "model_loaded": _model is not None, "tracker": TRACKER_PATH, "tracker_exists": os.path.exists(TRACKER_PATH), "evidence_directory": EVIDENCE_DIR, "route_points": store.route_data()["pointCount"]}
...
def detect(request: FrameRequest) -> dict[str, Any]:
    ...
    result = results[0]
    names, boxes = result.names, result.boxes
    track_ids = boxes.id.int().tolist() if boxes is not None and boxes.id is not None else []
    if boxes is not None:
        for index, box in enumerate(boxes):
            cls_id, confidence = int(box.cls.item()), float(box.conf.item())
...
def events() -> dict[str, Any]:
    return {"events": store.list_events(), "summary": store.summary(), "analytics": store.analytics(), "route": store.route_data()}
